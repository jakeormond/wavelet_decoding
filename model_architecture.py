...
def the_decoder(tg, show_summary=True):
    ...
    x = GaussianNoise(tg.gaussian_noise)(model_input) # add noise to mitigate overfitting (i.e. regularize)
    # timestep reductions
    for nct in range(0, tg.num_convs_tsr): # num_convs_tsr is number of downsampling steps)
